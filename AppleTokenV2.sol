...
contract AppleToken is Context, IERC20, Ownable {
    ...
    string private _name = "Apple Token";
    ...
    constructor () public {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xe0F3E2023CA907090ad4bb72d5EE127472D782F3);
    ...
    function setBurntFeePercent(uint256 burntFee) external onlyOwner() {
        _burntFee = burntFee;
    }
    ...
    function setBurntEnabled(bool _enabled) public onlyOwner {
        _burntEnabled = _enabled;
//        emit BurntEnabledUpdated(_enabled);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tBurnt = calculateBurntFee(tAmount); 
    ...
    function calculateBurntFee(uint256 _amount) private view returns (uint256) {
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _burntFee == 0) return;
    ...
    function _burntFeeValue(uint256 amount) private pure returns (uint256) {
        return calculateBurntFee(amount);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
     ...
     function _transferBurnt(address sender, address recipient,uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _rOwned[address(0)] =  _burntFeeValue(tAmount);
        _reflectFee(rFee, tFee);
        emit Transfer(sender, recipient, tTransferAmount);
        emit Transfer(sender, recipient, _burntFeeValue(tAmount));
    }
